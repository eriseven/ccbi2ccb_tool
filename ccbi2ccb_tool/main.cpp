...
//  ccbi2ccb_tool
//
//  Created by eric.li on 3/14/17.
//  Copyright © 2017 eric.li. All rights reserved.
//
...
bool readNodeRecurse(ccbiReader & ccbi, CCDictionary * pDict)
{
    ...
    // ÊúÄÁªàÂÜ≥ÂÆöÂü∫Á±ªÂíåËá™ÂÆö‰πâÁ±ª
    if (!bIsCustomClassIsStandardClass)
    {
        baseClass = decideStandardClass(pPropertiesArray);
        // Â∞ΩÂèØËÉΩÂå∫ÂàÜ"CCLayer"ËøòÊòØ"CCMenu"
...
//        pSequenceDict->insert(std::pair<std::string, CCObject *>("soundChannel", pSoundChannelDict));
    ...
    if (pTemplateDict != NULL) {
        ...
        
        {
            ...
            pResolutionDict->insert(std::pair<std::string, CCObject *>("ext", new CCString(" ")));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("width", new CCInteger(480)));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("height", new CCInteger(320)));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("name", new CCString("iPhone Landscape")));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("scale", new CCReal(1)));
        ...
        
        {
            ...
            pResolutionDict->insert(std::pair<std::string, CCObject *>("centeredOrigin", new CCBool(false)));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("ext", new CCString(" ")));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("width", new CCInteger(568)));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("height", new CCInteger(320)));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("name", new CCString("iPhone 5 Landscape")));
            pResolutionDict->insert(std::pair<std::string, CCObject *>("scale", new CCReal(1)));
            pResolutionsArray->push_back(pResolutionDict);
        }
...
int main(int argc, const char * argv[]) {
    ...
    std::string ccbiFile = "PbHUD.ccbi";
    if (argc > 1) {
        ccbiFile = argv[1];//ConvertToAString(argv[1]);
    }
